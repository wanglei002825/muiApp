...
var subpages = ['view/gongz/home.html', 
	'view/gongz/yingy.html','view/gongz/xiaox.html',
	'view/gongz/wod.html'];
var subpage_style = {
	top: '0px',
	bottom: '51px'
...
IndexJs={
	...
	initPage : function(){
			...
			mui('.mui-bar-tab').on('tap', 'a.mui-tab-item', function(e) {
				...
				if(!_subView) {
					if('view/gongz/wod.html' == targetTab){
						var style = {
							top: '0px',
							bottom: '51px'
						};
		...
		mui.back = function(){
			...
			if('indexGz' == nowselfId){
				...
				if (homeTab !== current) {//不是首页返回,则返回到首页页面
					...
					var webview = plus.webview.getWebviewById('view/gongz/home.html'); 
				    if(!webview) {
						webview = plus.webview.create('view/gongz/home.html', 'view/gongz/home.html', subpage_style);
						nowself.append(webview);
					}
			    	webview.show();
			    	// if("公众应用" == currentName){
			    	// 	plus.webview.getWebviewById('view/gongz/yingy.html').hide('none');
			    	// } else if("消息中心" == currentName){
			    	// 	plus.webview.getWebviewById('view/gongz/xiaox.html').hide('none');
			    	// } else if("我的" == currentName){
			    	// 	plus.webview.getWebviewById('view/gongz/wod.html').hide('none');
			    	// }
					if("我的" == currentName){
						plus.webview.getWebviewById('view/gongz/wod.html').hide('none');
					}
