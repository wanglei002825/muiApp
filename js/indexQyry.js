...
var subpages = ['view/qiyry/home.html', 
	'view/qiyry/yingy.html','view/qiyry/xiaox.html',
	'view/qiyry/wod.html'];
var subpage_style = {
	top: '0px',
	bottom: '51px'
...
IndexJs={
	...
	initPage : function(){
			...
			mui('.mui-bar-tab').on('tap', 'a.mui-tab-item', function(e) {
				...
				if(!_subView) {
					if('view/qiyry/wod.html' == targetTab){
						var style = {
							top: '0px',
							bottom: '51px'
						};
		...
		mui.back = function(){
			...
			if('indexQyry' == nowselfId){
				...
				if (homeTab !== current) {//不是首页返回,则返回到首页页面
					...
					var webview = plus.webview.getWebviewById('view/qiyry/home.html'); 
				    if(!webview) {
						webview = plus.webview.create('view/qiyry/home.html', 'view/qiyry/home.html', subpage_style);
						nowself.append(webview);
					}
			    	webview.show();
			    	if("行业应用" == currentName){
			    		plus.webview.getWebviewById('view/qiyry/yingy.html').hide('none');
			    	} else if("消息中心" == currentName){
			    		plus.webview.getWebviewById('view/qiyry/xiaox.html').hide('none');
			    	} else if("我的" == currentName){
			    		plus.webview.getWebviewById('view/qiyry/wod.html').hide('none');
			    	}
